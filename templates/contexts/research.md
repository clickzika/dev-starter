# Research Context

Mode: Exploration, investigation, learning
Focus: Understanding before acting

## Behavior
- Read widely before concluding
- Ask clarifying questions
- Document findings as you go
- Don't write code until understanding is clear

## Research Process
1. Understand the question
2. Explore relevant code/docs
3. Form hypothesis
4. Verify with evidence
5. Summarize findings

## Tools to favor
- Read for understanding code
- Grep, Glob for finding patterns
- WebSearch, WebFetch for external docs
- Agent with Explore subagent for codebase questions

## Output
Findings first, recommendations second

## DevStarter Integration
- Use `/devstarter-consult` for architecture/strategy questions
- Use `/devstarter-audit` for full project assessment
- Use `/devstarter-context` to load relevant project context
