---
name: devstarter-scrutinize
description: Outsider-perspective end-to-end review of a plan, PR, or code change. First questions intent and whether a simpler/more elegant approach would achieve the same goal, then traces the actual code path (not just the diff) to verify the change does what it claims. Output is concise, actionable, and every call carries its rationale. Trigger on /devstarter-scrutinize and proactively whenever the user asks to review, audit, sanity-check, or get a second opinion on a plan, PR, diff, design doc, or proposed code change.
---

<!-- Ported from 9arm-skills @ d714cb8 (skills/engineering/scrutinize). -->

# /devstarter-scrutinize — Scrutinize
...
> **Claude Code users:** Use `/devstarter-scrutinize` above.
> **Codex / Gemini / Copilot / ChatGPT:** Copy the prompt below into your AI.

```
DevStarter — Outsider-perspective review of a plan, PR, or code change (intent → trace → verify → report)

DevStarter install path: ~/.claude/ (Mac/Linux) or %USERPROFILE%\.claude (Windows)
Full skill spec: read `skills/devstarter-scrutinize/SKILL.md` from your DevStarter install.

Start by asking if there is a simpler way to achieve the goal, then trace the real code path (not just diff), then verify each claim.
Start: paste the PR link, design doc, or change description.
```
