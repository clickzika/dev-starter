---
name: devstarter-bug-postmortem
description: Write the canonical engineering record of a fixed bug — root cause, mechanism, fix, validation, and how it slipped through. Engineer-audience, code identifiers welcome. Use after a debug session lands a fix, before closing the ticket. Distinct from /devstarter-postmortem (incident response). Trigger on /devstarter-bug-postmortem, when the user says "write the bug post-mortem / RCA / root cause analysis" for a fixed bug, "document this fix", "write up the root cause", "close out this bug with a writeup", or hands you a fixed-and-validated bug and asks for the writeup.
---

<!-- Ported from 9arm-skills @ d714cb8 (skills/engineering/post-mortem). Renamed to devstarter-bug-postmortem to avoid collision with devstarter-postmortem (incident workflow). -->

# /devstarter-bug-postmortem — Bug Post-mortem
...
> **Not the same as `/devstarter-postmortem`.** That is a blameless **incident** post-mortem (SEV-1/SEV-2 customer-visible event with timeline + 5 Whys + action items). This skill is for the **engineering record of a single fixed bug** — RCA, mechanism, fix, validation. Use the incident workflow for outages; use this one for bug-fix writeups.

For the up-the-org version of this same content, hand the finished post-mortem to [`devstarter-management-talk`](../devstarter-management-talk/SKILL.md). They compose: bug-postmortem owns the engineering truth, management-talk reframes it for leadership.

## When to invoke

- "/devstarter-bug-postmortem"
- "write the bug post-mortem / RCA / root-cause analysis" *for a fixed bug*
...
- **Customer-visible outage / incident.** Use `/devstarter-postmortem` instead — those need timeline, blast radius, paging history, comms. This skill is bug-fix scope.
...
These map directly to `devstarter-debug-mantra` steps 1–4. If you came in via that skill, the breadcrumb ledger from step 4 is your raw material — pull from it.
...
- What tools cracked it (debugger, source tracing, knob enumeration, in-code instrumentation — the `devstarter-debug-mantra` step 2 cascade).
...
This is engineer-to-engineer. Different from `devstarter-management-talk`:
...
5. **Offer the management-talk handoff:** *"Want a leadership-flavored version? I can hand this to `devstarter-management-talk`."* Don't do it automatically.
...
- **Never strip code identifiers** in the engineering record. They are the index. The leadership reframe is `devstarter-management-talk`'s job, not yours.
...
```
DevStarter — Write the engineering record of a fixed bug (root cause, mechanism, fix, validation)

DevStarter install path: ~/.claude/ (Mac/Linux) or %USERPROFILE%\.claude (Windows)
Full skill spec: read `skills/devstarter-bug-postmortem/SKILL.md` from your DevStarter install.

Refuse to draft unless: reliable repro + known root cause + identified fix + validated fix.
Start: confirm all four required inputs, then produce the post-mortem in the structure above.
```
