---
name: devstarter-management-talk
description: Rewrite engineer-to-engineer content for engineering-org leadership (VPs, directors, PMs, release managers, execs in an engineering-savvy company) and shape it for the channel it is going to — JIRA comment, Slack post, async standup line, email, or meeting talking-points. Trigger on /devstarter-management-talk and when the user asks to write/rewrite for management / exec / VP / director / PM / release manager, asks for an "executive summary / leadership update / status update", says "make this less technical / less jargony", or asks for a slack / email / standup / meeting version of work originally written engineer-to-engineer.
---

<!-- Ported from 9arm-skills @ d714cb8 (skills/productivity/management-talk). -->

# /devstarter-management-talk — Management Talk
...
> **Claude Code users:** Use `/devstarter-management-talk` above.
> **Codex / Gemini / Copilot / ChatGPT:** Copy the prompt below into your AI.

```
DevStarter — Rewrite engineering content for leadership (JIRA / Slack / standup / email / meeting)

DevStarter install path: ~/.claude/ (Mac/Linux) or %USERPROFILE%\.claude (Windows)
Full skill spec: read `skills/devstarter-management-talk/SKILL.md` from your DevStarter install.

Confirm the channel first, then rewrite preserving product/JIRA/PR identifiers but stripping code identifiers.
Start: paste the engineering content + channel (JIRA / Slack / standup / email / meeting).
```
