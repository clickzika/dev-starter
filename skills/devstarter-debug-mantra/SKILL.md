---
name: devstarter-debug-mantra
description: Four-mantra debugging discipline — reproduce, trace the fail path, falsify the hypothesis, cross-reference every breadcrumb. Recite the mantra block verbatim at the start of any debugging session, then apply the four steps in order before proposing any fix. Trigger on /devstarter-debug-mantra and proactively whenever debugging starts — user reports a bug, says something is broken/throwing/failing, asks to debug/diagnose/investigate an issue, or pastes a stack trace or error log.
---

<!-- Ported from 9arm-skills @ d714cb8 (skills/engineering/debug-mantra). Upstream: https://github.com/efskap/9arm-skills (or local repo). -->

# /devstarter-debug-mantra — Debug Mantra
...
> **Claude Code users:** Use `/devstarter-debug-mantra` above.
> **Codex / Gemini / Copilot / ChatGPT:** Copy the prompt below into your AI.

```
DevStarter — Apply four-step debug discipline (reproduce → trace → falsify → breadcrumb)

DevStarter install path: ~/.claude/ (Mac/Linux) or %USERPROFILE%\.claude (Windows)
Full skill spec: read `skills/devstarter-debug-mantra/SKILL.md` from your DevStarter install.

Recite the mantra verbatim, then apply the four steps in order before proposing any fix.
Start: paste the bug, stack trace, or error you're debugging.
```
