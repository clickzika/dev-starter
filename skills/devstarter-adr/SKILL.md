...
> **Claude Code users:** Use `/devstarter-adr` above.
> **Codex / Gemini / Copilot / ChatGPT:** Copy the prompt below into your AI.

```
DevStarter — Create an Architecture Decision Record for a technical choice

DevStarter install path: ~/.claude/ (Mac/Linux) or %USERPROFILE%\.claude (Windows)
Full workflow / agent spec: read `sdlc/devstarter-adr.md` from your DevStarter install.
