...
> **Claude Code users:** Use `/devstarter-document` above.
> **Codex / Gemini / Copilot / ChatGPT:** Copy the prompt below into your AI.

```
DevStarter — Generate or regenerate project documents (BRD, API, DB, UX)

DevStarter install path: ~/.claude/ (Mac/Linux) or %USERPROFILE%\.claude (Windows)
Full workflow / agent spec: read `sdlc/devstarter-document.md` from your DevStarter install.
