...
- **Bilingual content (MANDATORY)** — ALL generated documents MUST contain both English and Thai for every text block using `<span class="lang-en">` / `<span class="lang-th">` pairs. This includes **all static chrome**: section titles (`<h2 class="section-title">`), sidebar TOC labels, `<h3>` subheadings, and table headers — not just filled content. A heading left English-only is a bug. Code blocks, file paths, and technical identifiers are NOT translated. Lang toggle + PDF export buttons are built into all templates.
- **Human-style Thai (MANDATORY)** — Thai text MUST read as natural, human-written Thai, NOT literal/word-for-word machine translation. Use idiomatic phrasing (`และ` not `&`, natural word order), keep widely-used English technical terms where Thai engineers normally keep them (e.g. `API`, `endpoint`, `commit`), and avoid stiff calques (e.g. prefer `ต้นตอของปัญหา` over a literal `สาเหตุราก`). When unsure, phrase it the way a Thai developer would say it aloud. See Bilingual Content Rule in `agents/shared/devstarter-agent-base.md`.
