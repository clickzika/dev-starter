...
- Skill name: `understand-anything:understand-domain` (plugin-namespaced form).
- If not found in the available-skills list, the plugin is not registered for this
  ...
  command: `/understand-domain`); a Claude Code restart after `/plugin install` is
  usually required.
- Forward the user's arguments unchanged.

The plugin's `domain-analyzer` agent extracts domains, flows, and steps. Nothing
